...
app.post("/register",async(req,res)=>{
    try{
    const {id,nname,dob,nstatus,clan,chakra,jname,village,nrank,mentor} = req.body;
    const data = await pool.request().query(
        `insert into shinobi values(${id},'${nname}','${dob}','${nstatus}','${clan}','${chakra}','${jname}','${village}','${nrank}',${mentor})
        select * from shinobi where ninjaId = ${id}`
    );
